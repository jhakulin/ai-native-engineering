...
The active context supplies task meaning but is not a source for recalculating measurements. Treat missing information as unavailable. Do not infer semantic conclusions solely from sizes, paths, command names, or categories. Do not override or reinterpret `primaryMeasuredCauseId`; it is selected only by deterministic analysis.
