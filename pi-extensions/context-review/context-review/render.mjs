export function formatStatsReport(report) {
  ...
  if (report.usage.integrity) {
    lines.push(`- Raw usage records: ${report.usage.integrity.rawUsageRecordCount}`);
    lines.push(`- Distinct observable model requests: ${report.usage.integrity.distinctObservableRequestCount}`);
    lines.push(`- Duplicate usage records: ${report.usage.integrity.duplicateUsageRecordCount}`);
    lines.push(`- Conflicting usage records: ${report.usage.integrity.conflictingUsageRecordCount}`);
    lines.push(`- Request identity: ${report.usage.integrity.identityRule}`);
  }
  ...
  lines.push("", "## Primary measurable cause");
  const primary = (report.measuredDrivers ?? []).find((driver) => driver.id === report.primaryMeasuredCauseId);
  if (primary) {
    lines.push(`- ${primary.name} — ${primary.id}`);
    lines.push(`- Target: ${primary.target}; impact: ${integer(primary.impact.value)} ${primary.impact.unit}${primary.impact.share === null ? "" : ` (${primary.impact.share}%)`}`);
  } else {
    lines.push("- Primary measurable cause: unavailable");
    lines.push(`- Reason: ${primaryUnavailableReason(report)}`);
  }

  lines.push("", "## Measured drivers");
  if (!(report.measuredDrivers ?? []).length) lines.push("- None supported by the available measurements.");
  else if (!primary) lines.push("- Unranked across different targets or without comparable attributable impact.");
  else if (report.measuredDrivers.some((driver) => driver.target !== primary.target || driver.scope !== primary.scope || driver.unit !== primary.unit)) {
    lines.push("- Primary cause is first; remaining drivers are not ranked across different targets, scopes, or units.");
  }
  for (const [index, driver] of (report.measuredDrivers ?? []).entries()) {
    lines.push(`${index + 1}. ${driver.name} — ${driver.id}`);
    lines.push(`   ${driver.description} [target=${driver.target}; ${driver.evidence}; ${driver.scope}]`);
...
function primaryUnavailableReason(report) {
  if ((report.usage?.integrity?.conflictingUsageRecordCount ?? 0) > 0) return "Conflicting usage records invalidate cumulative attribution.";
  if (report.usage?.cumulative?.input !== null) return "Preserved usage shows cumulative input, but does not attribute it to replay, fresh material, cache activity, or another comparable cause.";
  return "Available measurements do not establish comparable attributable causes in one target, scope, and unit.";
}
