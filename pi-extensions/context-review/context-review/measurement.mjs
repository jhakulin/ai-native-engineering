...
export function buildStatsReport(records, report, selection = {}) {
  ...
  const cumulativeInputAttribution = buildInputAttribution(agent, usage.cumulative);
  const latestInputAttribution = buildInputAttribution(agent, usage.latest);
  const outputAttribution = buildVisibleOutputAttribution(agent, usage.cumulative, cumulativeInputAttribution.processedInput, usage.reasoningSubsetComplete);
  const inputAttribution = {
    ...cumulativeInputAttribution,
    latestProcessedInput: latestInputAttribution.processedInput,
    ...outputAttribution,
  };
  const failureActivity = collectFailureActivity(records, agent);
  ...
  const metrics = [
    ...
    metric("usage.latest.input", "Latest provider-reported input", usage.latest.input, "tokens", SCOPES.LATEST_RESPONSE, usage.latest.input === null ? EVIDENCE.UNAVAILABLE : EVIDENCE.PROVIDER, usage.source),
    metric("usage.latest.processed_input", "Latest provider-reported processed input", inputAttribution.latestProcessedInput, "tokens", SCOPES.LATEST_RESPONSE, inputAttribution.latestProcessedInput === null ? EVIDENCE.UNAVAILABLE : EVIDENCE.PROVIDER, latestInputAttribution.source, latestInputAttribution.limitation),
    metric("usage.cumulative.input", "Cumulative provider-reported input", usage.cumulative.input, "tokens", SCOPES.CUMULATIVE, usage.cumulative.input === null ? EVIDENCE.UNAVAILABLE : EVIDENCE.PROVIDER, usage.source, usage.integrity.conflictingUsageRecordCount > 0 ? "Conflicting duplicate usage records make the cumulative value unavailable." : undefined),
    metric("usage.cumulative.processed_input", "Cumulative provider-reported processed input", inputAttribution.processedInput, "tokens", SCOPES.CUMULATIVE, inputAttribution.processedInput === null ? EVIDENCE.UNAVAILABLE : EVIDENCE.PROVIDER, inputAttribution.source, inputAttribution.limitation),
    metric("usage.cumulative.reprocessed_input", "Cumulative provider-reported reprocessed input", inputAttribution.reprocessedInput, "tokens", SCOPES.CUMULATIVE, inputAttribution.reprocessedInput === null ? EVIDENCE.UNAVAILABLE : EVIDENCE.PROVIDER, inputAttribution.source, inputAttribution.limitation),
    metric("usage.cumulative.fresh_input", "Cumulative provider-reported fresh input", inputAttribution.freshInput, "tokens", SCOPES.CUMULATIVE, inputAttribution.freshInput === null ? EVIDENCE.UNAVAILABLE : EVIDENCE.PROVIDER, inputAttribution.source, inputAttribution.limitation),
    metric("usage.cumulative.visible_output", "Cumulative provider-reported visible output", inputAttribution.visibleOutput, "tokens", SCOPES.CUMULATIVE, inputAttribution.visibleOutput === null ? EVIDENCE.UNAVAILABLE : EVIDENCE.PROVIDER, inputAttribution.outputSource, inputAttribution.outputLimitation),
    metric("usage.cumulative.input_output_tokens", "Cumulative processed input and visible output", inputAttribution.inputOutputTokens, "tokens", SCOPES.CUMULATIVE, inputAttribution.inputOutputTokens === null ? EVIDENCE.UNAVAILABLE : EVIDENCE.PROVIDER, inputAttribution.outputSource, inputAttribution.outputLimitation),
    failureActivity.available
      ? metric("activity.repeated_failed_operations", "Repeated explicitly failed operations", failureActivity.repeatedCount, "calls", SCOPES.CUMULATIVE, EVIDENCE.EXACT, failureActivity.source)
      : unavailableMetric("activity.repeated_failed_operations", "Repeated explicitly failed operations", "calls", SCOPES.CUMULATIVE, failureActivity.source, failureActivity.limitation),
    metric("session.model_calls", "Observable model calls", usage.calls, "calls", SCOPES.CUMULATIVE, usage.calls > 0 ? EVIDENCE.PROVIDER : EVIDENCE.UNAVAILABLE, usage.callSource, usage.calls === 0 ? "No provider usage records were preserved." : undefined),
  ...
  const result = {
    ...
    usage: {
      ...
      reasoningSubsetComplete: usage.reasoningSubsetComplete,
  ...
  result.primaryMeasuredCauseId = selectPrimaryMeasuredCause(result);
  result.measuredDrivers = selectMeasuredDrivers(result);
  return result;
}
...
export function attachObservedRequest(statsReport, payload, observedAt, limitation = undefined) {
  ...
  report.primaryMeasuredCauseId = selectPrimaryMeasuredCause(report);
  report.measuredDrivers = selectMeasuredDrivers(report);
...
export function buildInMemoryStatsReport({ cwd, systemPrompt, model, effort }) {
  ...
  const metrics = [
    ...
    unavailableMetric("usage.latest.input", "Latest provider-reported input", "tokens", SCOPES.LATEST_RESPONSE, "Session not persisted", "Provider response usage is unavailable."),
    unavailableMetric("usage.latest.processed_input", "Latest provider-reported processed input", "tokens", SCOPES.LATEST_RESPONSE, "Session not persisted", "Provider response usage is unavailable."),
    unavailableMetric("usage.cumulative.input", "Cumulative provider-reported input", "tokens", SCOPES.CUMULATIVE, "Session not persisted", "Provider response usage is unavailable."),
    unavailableMetric("usage.cumulative.processed_input", "Cumulative provider-reported processed input", "tokens", SCOPES.CUMULATIVE, "Session not persisted", "Provider response usage is unavailable."),
    unavailableMetric("usage.cumulative.reprocessed_input", "Cumulative provider-reported reprocessed input", "tokens", SCOPES.CUMULATIVE, "Session not persisted", "Provider response usage is unavailable."),
    unavailableMetric("usage.cumulative.fresh_input", "Cumulative provider-reported fresh input", "tokens", SCOPES.CUMULATIVE, "Session not persisted", "Provider response usage is unavailable."),
    unavailableMetric("usage.cumulative.visible_output", "Cumulative provider-reported visible output", "tokens", SCOPES.CUMULATIVE, "Session not persisted", "Provider response usage is unavailable."),
    unavailableMetric("usage.cumulative.input_output_tokens", "Cumulative processed input and visible output", "tokens", SCOPES.CUMULATIVE, "Session not persisted", "Provider response usage is unavailable."),
    unavailableMetric("activity.repeated_failed_operations", "Repeated explicitly failed operations", "calls", SCOPES.CUMULATIVE, "Session not persisted", "Failure activity is unavailable."),
  ...
  const result = {
    ...
    metrics,
    usage: {
      latest: emptyUsage(), cumulative: emptyUsage(), units: "tokens", latestScope: SCOPES.LATEST_RESPONSE,
      cumulativeScope: SCOPES.CUMULATIVE, evidence: EVIDENCE.UNAVAILABLE, source: "Session not persisted",
      integrity: usageIntegrity(0, 0, 0, 0, "no persisted usage records"),
      reasoningSubsetComplete: false,
      attribution: { processedInput: null, reprocessedInput: null, freshInput: null, source: "Session not persisted", limitation: "Provider response usage is unavailable.", latestProcessedInput: null, visibleOutput: null, inputOutputTokens: null, outputSource: "Session not persisted", outputLimitation: "Provider response usage is unavailable." },
    },
    composition,
    contributors,
    activity: { available: false, repeatedCount: 0, repeatedFailures: [], unidentifiedFailureCount: 0, source: "Session not persisted", limitation: "Failure activity is unavailable." },
    measuredDrivers: [],
    primaryMeasuredCauseId: null,
    observedRequest: null,
    limitations: [
...
export function selectPrimaryMeasuredCause(report) {
  if ((report?.usage?.integrity?.conflictingUsageRecordCount ?? 0) > 0) return null;
  const hasCumulativeInput = Number.isFinite(report?.usage?.attribution?.processedInput) || Number.isFinite(report?.usage?.cumulative?.input);
  const target = hasCumulativeInput ? "cumulative_input_consumption" : "active_context_occupancy";
  const unit = hasCumulativeInput ? "tokens" : "bytes";
  const candidates = (report?.measuredDrivers ?? []).filter((driver) =>
    driver.target === target
    && driver.impact?.unit === unit
    && Number.isFinite(driver.impact?.value)
  );
  if (new Set(candidates.map((driver) => driver.scope)).size !== 1) return null;
  candidates.sort(compareCauseImpact);
  return candidates[0]?.id ?? null;
}

function selectMeasuredDrivers(report) {
  const drivers = [...(report?.measuredDrivers ?? [])];
  const selected = [];
  const selectedIds = new Set();
  const representedTargets = new Set();
  const add = (driver) => {
    if (!driver || selectedIds.has(driver.id) || selected.length >= 3) return;
    selected.push(driver);
    selectedIds.add(driver.id);
    representedTargets.add(driver.target);
  };
  const primary = drivers.find((driver) => driver.id === report.primaryMeasuredCauseId);
  add(primary);
  const targetPriority = ["activity", "cumulative_input_consumption", "cumulative_input_output_consumption", "active_context_occupancy"];
  for (const target of targetPriority) {
    if (representedTargets.has(target)) continue;
    const candidates = drivers.filter((driver) => driver.target === target && !selectedIds.has(driver.id)).sort(compareCauseImpact);
    add(candidates[0]);
  }
  for (const target of targetPriority) {
    for (const driver of drivers.filter((item) => item.target === target && !selectedIds.has(item.id)).sort(compareCauseImpact)) add(driver);
  }
  return selected;
}

function compareCauseImpact(left, right) {
  const leftShare = Number.isFinite(left.impact?.share) ? left.impact.share : -1;
  const rightShare = Number.isFinite(right.impact?.share) ? right.impact.share : -1;
  const leftValue = Number.isFinite(left.impact?.value) ? left.impact.value : -1;
  const rightValue = Number.isFinite(right.impact?.value) ? right.impact.value : -1;
  return rightShare - leftShare
    || rightValue - leftValue
    || evidenceRank(right.evidence) - evidenceRank(left.evidence)
    || (right.occurrenceCount ?? 0) - (left.occurrenceCount ?? 0)
    || left.id.localeCompare(right.id);
}

function evidenceRank(evidence) {
  return { exact_observed: 4, exact_serialized_local_representation: 3, provider_reported: 3, reconstructed: 2, unavailable: 0 }[evidence] ?? 1;
}
...
function collectUsage(records, agent) {
  ...
  if (agent === "codex") {
    ...
    return usageResult(samples, lastTotal ? normalizeUsage(lastTotal) : sumUsage(samples), "event_msg.payload.info.last_token_usage / total_token_usage", usageIntegrity(samples.length, samples.length, 0, 0, "token-count event ordinal"));
  }
  if (agent === "claude") return collectClaudeUsage(records);
  ...
  return usageResult(samples, sumUsage(samples), source, usageIntegrity(samples.length, samples.length, 0, 0, "assistant message record ordinal"));
}

function collectClaudeUsage(records) {
  const raw = records.filter((record) => record?.type === "assistant" && record.message?.usage);
  const groups = new Map();
  raw.forEach((record, index) => {
    const requestId = typeof record.requestId === "string" && record.requestId ? record.requestId : null;
    const key = requestId ? `request:${requestId}` : `record:${index}`;
    const group = groups.get(key) ?? { samples: [], lastIndex: index, conflicting: false };
    group.samples.push(normalizeUsage(record.message.usage));
  ...
  for (const group of groups.values()) {
    ...
    group.conflicting = group.samples.some((sample) => !sameUsage(sample, first));
    if (group.conflicting) conflicts += 1;
    else samples.push(first);
  }
  const duplicates = raw.length - groups.size;
  const integrity = usageIntegrity(raw.length, groups.size, duplicates, conflicts, "requestId when preserved; assistant usage-record ordinal otherwise");
  const result = usageResult(samples, conflicts > 0 ? emptyUsage() : sumUsage(samples), "assistant.message.usage", integrity);
  const latestGroup = [...groups.values()].sort((left, right) => right.lastIndex - left.lastIndex)[0];
  result.latest = !latestGroup || latestGroup.conflicting ? emptyUsage() : latestGroup.samples[0];
  if (raw.some((record) => !record.requestId)) result.limitations.push("Some Claude usage records lacked requestId; those records were counted individually and may contain undetectable duplicates.");
  if (conflicts > 0) result.limitations.push("Conflicting Claude usage blocks shared a requestId; cumulative usage and primary-cause selection are unavailable.");
  return result;
}
...
function usageIntegrity(rawUsageRecordCount, distinctObservableRequestCount, duplicateUsageRecordCount, conflictingUsageRecordCount, identityRule) {
  return { rawUsageRecordCount, distinctObservableRequestCount, duplicateUsageRecordCount, conflictingUsageRecordCount, identityRule };
}

function collectFailureActivity(records, agent) {
  if (!["claude", "pi"].includes(agent)) {
    return { available: false, repeatedCount: 0, repeatedFailures: [], unidentifiedFailureCount: 0, source: "No verified explicit failure-status adapter", limitation: "Repeated failure activity is unavailable for this harness." };
  }
  const failedOperationIds = new Set();
  let unidentifiedFailureCount = 0;
  records.forEach((record) => {
    if (agent === "claude") {
      const content = Array.isArray(record?.message?.content) ? record.message.content : [];
      content.forEach((block) => {
        if (block?.type !== "tool_result" || block.is_error !== true) return;
        if (typeof block.tool_use_id === "string" && block.tool_use_id) failedOperationIds.add(block.tool_use_id);
        else unidentifiedFailureCount += 1;
      });
    } else if (record?.type === "message" && record.message?.role === "toolResult" && record.message.isError === true) {
      if (typeof record.message.toolCallId === "string" && record.message.toolCallId) failedOperationIds.add(record.message.toolCallId);
      else unidentifiedFailureCount += 1;
    }
  });
  const repeatedCount = failedOperationIds.size >= 2 ? failedOperationIds.size : 0;
  return {
    available: true,
    repeatedCount,
    repeatedFailures: repeatedCount ? [{ class: "explicit_tool_result_error", count: repeatedCount, unit: "calls", evidence: EVIDENCE.EXACT }] : [],
    unidentifiedFailureCount,
    source: agent === "claude" ? "Claude tool_result.is_error linked by tool_use_id" : "Pi toolResult message.isError linked by toolCallId",
    limitation: unidentifiedFailureCount > 0
      ? `${unidentifiedFailureCount} explicit failure record(s) lacked a verified operation identifier and were excluded.`
      : repeatedCount ? "Explicit failures do not establish that the operations were retries or unnecessary." : null,
  };
}

function buildInputAttribution(agent, cumulative) {
  if (!Number.isFinite(cumulative?.input)) {
    return { processedInput: null, reprocessedInput: null, freshInput: null, source: "Provider input usage unavailable", limitation: "Provider input usage is unavailable for this measurement scope." };
  }
  if (agent === "codex") {
    const cachedInput = Number.isFinite(cumulative.cachedInput) && cumulative.cachedInput >= 0 && cumulative.cachedInput <= cumulative.input ? cumulative.cachedInput : null;
    return {
      processedInput: cumulative.input,
      reprocessedInput: cachedInput,
      freshInput: cachedInput === null ? null : cumulative.input - cachedInput,
      source: "Codex total input_tokens with cached_input_tokens as a reported subset",
      limitation: cachedInput === null ? "Codex cached input is unavailable or inconsistent with total input." : null,
    };
  }
  if (!Number.isFinite(cumulative.cacheRead) || !Number.isFinite(cumulative.cacheWrite)) {
    return {
      processedInput: null,
      reprocessedInput: null,
      freshInput: null,
      source: `${agent} input plus separately reported cache-read and cache-write tokens`,
      limitation: `${agent} processed-input attribution requires complete input, cache-read, and cache-write usage in this measurement scope.`,
    };
  }
  return {
    processedInput: cumulative.input + cumulative.cacheRead + cumulative.cacheWrite,
    reprocessedInput: cumulative.cacheRead,
    freshInput: cumulative.input,
    source: `${agent} input plus separately reported cache-read and cache-write tokens`,
    limitation: null,
  };
}

function buildVisibleOutputAttribution(agent, cumulative, processedInput, reasoningSubsetComplete) {
  if (agent === "claude") {
    return {
      visibleOutput: null,
      inputOutputTokens: null,
      outputSource: "Claude output_tokens without a separate preserved reasoning-output field",
      outputLimitation: "Visible output is unavailable because preserved Claude usage does not separate reasoning output.",
    };
  }
  const output = cumulative?.output;
  const reasoning = cumulative?.reasoningOutput;
  if (!reasoningSubsetComplete || !Number.isFinite(processedInput) || !Number.isFinite(output) || !Number.isFinite(reasoning) || reasoning < 0 || reasoning > output) {
    return {
      visibleOutput: null,
      inputOutputTokens: null,
      outputSource: `${agent} output minus separately reported reasoning output`,
      outputLimitation: "Visible-output attribution requires complete output and a valid separately reported reasoning-output subset.",
    };
  }
  const visibleOutput = output - reasoning;
  return {
    visibleOutput,
    inputOutputTokens: processedInput + visibleOutput,
    outputSource: `${agent} output minus separately reported reasoning output`,
    outputLimitation: null,
  };
}
...
function usageResult(samples, cumulative, source, integrity) {
  const latest = samples.at(-1) ?? emptyUsage();
  return {
    latest,
    cumulative: cumulative ?? emptyUsage(),
    calls: integrity.distinctObservableRequestCount,
    callSource: `${source}; ${integrity.identityRule}`,
    source,
    integrity,
    reasoningSubsetComplete: samples.length > 0 && samples.every((sample) =>
      Number.isFinite(sample.output) && Number.isFinite(sample.reasoningOutput)
      && sample.reasoningOutput >= 0 && sample.reasoningOutput <= sample.output
    ),
    limitations: integrity.rawUsageRecordCount ? [] : ["Provider response usage is unavailable in the preserved session."],
  };
}
...
function sumUsage(samples) {
  ...
  for (const key of Object.keys(result)) {
    if (samples.every((sample) => Number.isFinite(sample[key]))) result[key] = samples.reduce((sum, sample) => sum + sample[key], 0);
  }
  return result;
}
