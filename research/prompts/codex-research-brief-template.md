...
- Prefer practical examples and measured optimization over additional abstract guidance. When a source supports an operating pattern, extract its actors, trigger, inputs, systems, steps, authority boundaries, outputs, verification, observed cost or benefit, and failure conditions.
- For repository documentation or context optimization, look for before-and-after evidence such as tokens, tool calls, latency, retries, clarification cycles, review effort, correctness, or task completion. Do not claim an optimization from token reduction alone when quality or rework worsens.
- For Jira, GitHub, Confluence, Microsoft Teams, or similar systems, distinguish the collaboration surface from the authoritative source, preserve permissions and provenance, and keep consequential product, architecture, and acceptance decisions under human authority.
