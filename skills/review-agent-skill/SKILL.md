...
3. Inspect the skill in context.
   - If reviewing an existing repository skill, list `skills/*/SKILL.md`, compare skill names and frontmatter descriptions to identify likely overlap or naming conflicts, and read `AGENTS.md` for repository-level requirements. For each plausible overlap, read the relevant skill bodies and compare trigger boundaries, workflow constraints, tool choices, permissions, output requirements, and definitions of done. Do not infer compatibility from names and descriptions alone.
   - If the skill was copied, forked, or adapted and its source is available, inspect the source and material local changes. Check whether the package makes intentional changes to the source's decisions about tools, acceptable shortcuts, data access, and what counts as complete. If provenance matters to trust or maintenance but cannot be verified, state that as a limitation rather than assuming the source is safe or current.
   - Read the raw frontmatter and inspect files in the skill directory that are referenced by `SKILL.md` or can be loaded or executed by its workflow. Do not rely on a rendered summary or marketplace view, which may omit behavior-relevant metadata. Treat `SKILL.md`, references, examples, scripts, configuration, and assets as one package for review; note files that change behavior, access, or data handling.
   - Inventory declared and implicit capabilities, including allowed tools, hooks, scripts, network access, credential or sensitive-data access, and state-changing actions. If the package came from outside the trusted repository, record the source and version used for review and check that the inspected content matches it.
   - Record whether package provenance/version, raw routing metadata, and required capability evidence are present. Treat missing evidence as a limitation or blocker, not a pass; where hooks or logs exist, verify skill activation and side-effectful tool use are observable.
   - If reviewing a proposal, evaluate only what is present and call out missing information as an assumption.

4. Produce findings.
   - Report issues that affect invocation, scope, workflow execution, guardrails, output quality, verification, duplication, cross-skill compatibility, context load, provenance, or maintainability.
   ...
   - Test routing separately from execution: include at least one in-scope request and one nearby false positive, then inspect whether side-effectful branches have an explicit read-only, approval, or escalation boundary.
...
- Do not treat installation or a valid directory structure as evidence that the skill is trustworthy, invocable, or effective.
