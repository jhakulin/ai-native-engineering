...
8. Before considering the skill complete, review the full package against the guideline. Check raw frontmatter and trigger boundaries, referenced or executable files, declared capabilities and side effects, provenance, positive and nearby false-positive routing examples, and approval or escalation behavior for side-effectful branches. Treat missing provenance or capability evidence as unresolved rather than safe.
9. Validate the repository with `node scripts/validate-repo.js`.
