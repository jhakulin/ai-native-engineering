...
Cause definitions, usage-record integrity, primary-cause selection, and the
evidence contract are defined in
[`context-efficiency-cause-diagnosis.md`](context-efficiency-cause-diagnosis.md).
This document defines how the advisor turns those findings into action.
...
- Evidence: 884,000 cached or cache-read tokens, 71% of 1,240,000 processed input tokens

Measured drivers

1. Context reprocessing — driver.context_reprocessing
   Evidence: 884,000 cached or cache-read tokens across 18 observable calls
   Limitation: cache use does not establish that retained context was unnecessary

2. Test output — driver.large_active_contributor
   Evidence: contributor tool-results:tool-result:1 occupies 81,204 serialized bytes
   Share: 15% of the latest observed payload

3. Input processed across multiple calls — driver.history_replay
   Evidence: 1,240,000 processed input tokens across 18 observable calls
   Limitation: preserved usage does not isolate replayed history from new input

Recommended actions

1. Return only failing integration-test output from tool `test`.
   Evidence addressed: driver.large_active_contributor and contributor tool-results:tool-result:1.
