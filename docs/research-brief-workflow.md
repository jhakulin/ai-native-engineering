...
Source exclusion does not depend on merging. Query planning and candidate gathering combine the checked-out source history with history from this repository's research PRs for the same target, including open, merged, and closed-unmerged PRs. Once a run has published its PR, the next run (including another run on the same day) excludes its recently processed sources under the target's existing reuse policy. The current 90-day window is unchanged; the latest recorded use wins when histories overlap. History is read at each PR's head commit, so deleting its branch does not require merging the PR to retain exclusion.

The combined history is used in memory, not copied into the new run's committed history. These reads use GitHub CLI with the workflow's existing contents/pull-request permissions. A failed PR-history read stops the step rather than silently selecting repeated sources. Runs that have not published a PR are not included. Local runs remain local-history-only unless `RESEARCH_INCLUDE_PR_HISTORY=true`, `GITHUB_REPOSITORY=owner/repo`, and GitHub CLI authentication are supplied.
