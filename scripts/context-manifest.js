...
function manifestFiles() {
  ...
  for (const file of tracked) {
    if ((file.startsWith("docs/") || file.startsWith("guidelines/") || file.startsWith("skills/") || file.startsWith("strategies/") || file.startsWith("ideas/")) && /\.md$/i.test(file)) files.add(file);
  }
  return [...files].sort();
}
...
function validateManifest(manifest = readManifest()) {
  ...
  for (const file of actualFiles) {
    ...
    if (!(requiredFiles.includes(file) || /^(docs|guidelines|skills|strategies|ideas)\/.*\.md$/i.test(file))) errors.push(`file is outside the allowed context: ${file}`);
  }
  return errors;
}
