...
const requestTimeoutMs = Number(process.env.OPENAI_REQUEST_TIMEOUT_MS || 300000);
const maxDescriptionChars = Number(process.env.RESEARCH_CANDIDATE_DESCRIPTION_MAX_CHARS || 1200);
...
async function main() {
  ...
  const selectionCandidates = eligibleCandidates.map((candidate) => ({
    ...candidate,
    description: String(candidate.description || "").slice(0, maxDescriptionChars),
  }));
