...
Capacity released by AI is an explicit operating decision. Teams should decide where that capacity creates the most value instead of automatically increasing feature volume. Strong candidates include product quality, reliability, performance, security, customer problems, engineering-system improvements, and consequential technical debt. Evaluate the reinvestment through existing product and engineering outcomes rather than a new measure of AI output.
...
- Share effective workflows with the team using concrete examples and outcome evidence.

The ownership, provenance, evaluation, approval, distribution, and retirement model for a shared registry is described in `ai-asset-registry.md`.
...
- Teams decide deliberately whether capacity released by AI should improve quality, reliability, customer outcomes, engineering-system health, or delivery throughput.
- Teams with demonstrated improvements show what changed and help other teams adapt the practice to their own workflow and constraints.
...
The system architecture for these workflows is described in `governed-agentic-development.md`. Phase 3 concerns organizational adoption and governance; the companion document defines the task, control-plane, worker, evidence, and acceptance boundaries.
...
- Convert repeated review feedback and significant failure patterns into appropriately scoped tests, CI checks, lints, monitors, instructions, architecture decision records, reviewer-agent checks, or policy gates, following the guardrails and control-loops guidance in `ai-harness-engineering.md`.
...
- For workflows that change contracts, requirements, or other durable artifacts, preserve a trace from intent and constraints through the agent proposal, verification evidence, human decision, and approved baseline.
- Select the smallest verification that proves the relevant invariant, then broaden checks when the change has wider impact or uncertainty; do not treat a passing local check as system-level proof.
...
- Team-to-team demonstration and coaching based on evidenced workflow improvements.
