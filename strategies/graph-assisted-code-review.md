...
Prefer a compact provenance header in the slice:

- `source_commit` or ref
- `indexed_at` / `updated_at`
- indexed-file and parser coverage
- refreshed paths and relationship types
- unresolved or inferred relationships
- verification status and fallback decision

For incremental graph updates, do not report a clean slice merely because the changed file was indexed; record whether dependent edges, tests, configuration, and generated artifacts were refreshed. A smaller context is useful only if the reviewer can see what was included, what was not, and how the claims were verified.
