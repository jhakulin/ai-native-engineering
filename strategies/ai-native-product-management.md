...
When the team cannot resolve an important product question abstractly, use a small set of contrasting prototypes as decision evidence before committing detailed scope.
...
The agent recommends; the human decides. Backlog changes affect priority, customer commitments, team focus, and delivery cost. A bounded intake loop collects structured input, produces an evidence-linked recommendation, requires product-owner review, and records the final decision. For repeated triage, make the lifecycle explicit: give each item exactly one category (such as bug or enhancement) and exactly one mutually exclusive state (such as needs triage, needs information, ready for agent, needs human implementation, or won't fix). Treat `ready for agent` as a gate, not a confidence label: move an item there only after a human has supplied a brief with context, acceptance expectations, and any needed reproduction or diagnosis, plus an explicit scope or out-of-scope decision. If ownership or next action is uncertain, route it to human review, record the recommendation and confidence, and preserve the reason in the item.
...
Start with suggestions and drafts. For workflows that may change durable state, introduce autonomy in stages: classify and route first; automate a narrow, reversible action only after representative evaluation, explicit policy, a failure and exception path, an audit trail, and a named human owner are in place. Scope auto-action to a bounded workflow or project rather than enabling it globally, and keep flagged or low-confidence cases in human review. Automated routing, labeling, or approval should remain reversible and visible.
