...
Agents may propose designs and make reversible implementation decisions inside an approved task contract. Humans retain authority for product intent, consequential architecture, risk acceptance, and final acceptance.
...
The required specificity depends on the work mode. During exploration, keep implementation details intentionally open while making the problem, non-negotiable constraints, and evaluation signal clear. During delegated implementation, resolve ambiguity that could change product intent, system boundaries, risk, or acceptance evidence. Ambiguity is useful in the solution space, not in authority, safety, or the definition of success.
...
Review intensity and granularity should follow risk. For lower-risk generated code, concentrate human attention on behavior, system shape, interfaces, dependency changes, architectural coherence, tests, observability, and recovery. Inspect individual lines as needed to validate those claims. If the code is core, sensitive, broadly coupled, or difficult to reverse, review every meaningful change. Low-risk prototypes may allow lighter review.
...
When the main uncertainty is experiential or architectural, produce two or three materially different runnable candidates rather than one elaborate proposal. Compare them using a small, fixed evaluation lens such as user fit, interface clarity, architectural coherence, operational risk, and implementation cost. Preserve the selected rationale and reusable learning; discard candidate code unless it independently meets production standards.
