...
Prefer interfaces with inspectable state, stable identifiers, compact structured input and output, explicit side-effect semantics, deterministic success and failure signals, actionable errors, and, where relevant, dry-run and idempotent operation. Agent-operable systems need not be agent-specific; they should be reliably automatable and understandable by both humans and machines.
