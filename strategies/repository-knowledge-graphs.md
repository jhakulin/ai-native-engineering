...
Use the retrieval mode that matches the question. Structural graph traversal should answer explicit relationship questions such as callers, dependents, paths, and tested-by links. Semantic or vector retrieval can find relevant documents, concepts, or candidate entities when names are incomplete. A hybrid workflow may use semantic retrieval to propose candidates and structural traversal to verify relationships, but similarity alone must not create authoritative edges. Report which mode produced each item and keep the verification boundary visible.

## Graph Update And Decision Provenance

Treat the graph as a versioned projection of repository state, not as an independent source of truth. Any graph-backed review or agent context should name the source commit or ref, indexed coverage, extraction or update time, and unsupported or unresolved areas. For incremental updates, record which changed paths and relationship types were refreshed; if coverage is unknown, mark the slice stale or incomplete and fall back to source-first inspection.

When graph context influences a review, documentation update, or agent action, preserve the selected slice, source paths, extraction method, confidence, and verification result. Keep source-extracted relationships separate from semantic or inferred links, and require human validation before inferred links become authoritative. This makes graph-backed decisions explainable and lets reviewers challenge or reproduce them.
...
General guidance for choosing retrieval modes, compiling task context, enforcing authorization, preserving provenance, and evaluating context systems is provided in `agent-context-systems.md`. This document retains the specialized requirements for repository graph construction and use.
...
Expose graph access through a bounded task-specific context surface rather than unrestricted graph exploration. A surface may combine semantic retrieval to propose candidate nodes or documents with structural traversal to verify relationships, and should return the source commit or ref, indexed coverage, freshness, relation type, extraction method, confidence, and unresolved or ambiguous areas. It should also name the smallest source set needed for verification and fall back to source-first inspection when the graph is stale or incomplete.

A hook or agent policy that requires a graph query before direct file reads is a workflow experiment, not a trust boundary. Measure it against a source-first baseline for missed paths, false positives, context use, latency, and maintenance cost, and preserve a fallback when the graph cannot answer. Do not let enforcement turn a graph projection into proof.
