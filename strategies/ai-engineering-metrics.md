...
It defines what an organization should monitor and report. Detailed methodology for designing representative agent evaluations and deciding whether evidence justifies release or increased autonomy is provided in `agent-workflow-evaluation.md`.
...
The team-discovery and continuous-feedback approach in `ai-harness-team-discovery-and-feedback.md` can establish qualitative baselines and identify which existing measures are useful for a particular team or workflow.
...
- Time waiting for product, architecture, or acceptance decisions.
- Number of human handoffs between ready intent and accepted outcome.
- Active agent work in progress per reviewer or decision owner.
- Time required to reconstruct context when switching between agent tasks.
- A lightweight team signal on whether the current agent-assisted pace is sustainable.
...
For PM-facing agent workflows, pair those flow measures with decision-quality measures: the share of inputs with traceable and sufficiently fresh evidence, agreement with expected categories on a representative evaluation set, recommendation acceptance/change/rejection rates, escalation for missing context, clarification or rework after handoff, cost per accepted decision, and rollback or exception counts for bounded write actions. Keep output quality, human intervention, and side-effect containment visible separately; adoption or activity volume does not demonstrate product judgment quality.
...
- First-pass acceptance and rework rate by task family and risk class.
- Time from ready to review and time waiting for human review.
- Blocked, abandoned, duplicate, or stale-claim rate.
- Confirmed versus unsupported automated-review findings.
- Cost, latency, and human intervention per accepted change.
- Recurrence of failure modes believed to be controlled.
...
For parallel workflows, record the concurrency level and resulting review queue. Increasing worker count is useful only while accepted-change throughput improves without disproportionate conflicts, rework, review delay, or loss of human understanding. The useful limit is the system's sustainable comprehension and acceptance capacity, not the maximum number of agents that can run.
...
`agent-pipeline-optimization.md` explains how to use these measures with flow, waiting time, rework, work in progress, review capacity, and quality-adjusted accepted throughput when changing a workflow.
