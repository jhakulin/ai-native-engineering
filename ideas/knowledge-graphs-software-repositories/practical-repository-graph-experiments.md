...
- for questions that mix code structure with natural-language concepts, a hybrid surface that uses semantic retrieval to propose candidate documents or entities and structural traversal to verify relationships

Keep the repository state, task, agent, model, and verification expectations fixed. For each run, record the graph commit, indexed coverage, query or tool used, retrieval mode, context size, latency, setup or refresh cost, source files inspected, useful paths found, missed relationships, false positives, and human corrections. Keep semantic candidates separate from verified graph edges and record whether each included relationship was extracted, manually declared, or inferred.
